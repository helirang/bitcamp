...
namespace ev_01_
{
    public partial class Form1 : Form
    {
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            //    randomNum = r.Next(10);
            //    if (i == 0)
            //    { NumGoal[i] = r.Next(10); i++; }
            //    else
            //    {
            //        if(i==1)
            //        { if(NumGoal[0] != randomNum)
            //            { NumGoal[i] = randomNum; i++; }
            //        }
            //        if (i == 2)
            //        {
            //            if (NumGoal[0] != randomNum && NumGoal[1] != randomNum)
            //            { NumGoal[i] = randomNum; i++; }
            //        }
            //    }
            //}
            ...
            while (this.isRecv)
            {
                try
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        for (int j = 0; j < 3; j++)
                        {
                            if (NumGoal[i] == Convert.ToInt32(data[j])-48)
                            {
                                if (i == j)
                                { strike++; }
                                else
                    ...
                    for(int i = 0; i < 3; i++)
                    {
                        chk = chk + NumGoal[i].ToString();
                    }
                    //SendAllChat(socket, chk);

                    //*Echo Server 기능
                    sw.WriteLine(chk);
                    sw.Flush();

                    AddLogListBox("→ Client Echo : " + chk);
                }
                catch (Exception ex)
                {
